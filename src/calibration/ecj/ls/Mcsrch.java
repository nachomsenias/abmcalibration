...
public class Mcsrch {
	/*
	 * RISO: an implementation of distributed belief networks. Copyright (C) 1999,
	 * Robert Dodier.
	 *
	 * This program is free software; you can redistribute it and/or modify it under
	 * either option (1) or (2) below. (1) The GNU General Public License as
	 * published by the Free Software Foundation; either version 2 of the License,
	 * or (at your option) any later version. (2) The Apache license, version 2.
	 *
	 ...
	 * Place, Suite 330, Boston, MA, 02111-1307, USA, or visit the GNU web site,
	 * www.gnu.org.
	 */
	...
	private static double dg = 0, dgm = 0, dginit = 0, dgtest = 0, dgx[] = new double[1], dgxm[] = new double[1],
	...
	/**
	 * Minimize a function along a search direction. This code is a Java translation
	 * of the function <code>MCSRCH</code> from <code>lbfgs.f</code>, which in turn
	 * is a slight modification of the subroutine <code>CSRCH</code> of More' and
	 * Thuente. The changes are to allow reverse communication, and do not affect
	 ...
	 * clean-up; in particular, array indices start at 0 instead of 1. Most of the
	 * comments from the Fortran code have been pasted in here as well.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * At each stage this function updates an interval of uncertainty with endpoints
	 * <code>stx</code> and <code>sty</code>. The interval of uncertainty is
	 * initially chosen so that it contains a minimizer of the modified function
	 * 
	 * <pre>
	 ...
	 * is chosen so that it contains a minimizer of <code>f(x+stp*s)</code>.
	 * <p>
	 *
	 ...
	 * </pre>
	 * 
	 * If <code>ftol</code> is less than <code>gtol</code> and if, for example, the
	 * function is bounded below, then there is always a step which satisfies both
	 * conditions. If no step can be found which satisfies both conditions, then the
	 * algorithm usually stops when rounding errors prevent further progress. In
	 * this case <code>stp</code> only satisfies the sufficient decrease condition.
	 * <p>
	 *
	 * @author Original Fortran version by Jorge J. More' and David J. Thuente as
	 *         part of the Minpack project, June 1983, Argonne National Laboratory.
	 ...
	 *
	 * @param x
	 *            On entry this contains the base point for the line search. On exit
	 *            it contains <code>x + stp*s</code>.
	 *
	 * @param f
	 ...
	 *
	 * @param g
	 *            On entry this contains the gradient of the objective function at
	 *            <code>x</code>. On exit it contains the gradient at
	 *            <code>x + stp*s</code>.
	 *
	 * @param s
	 ...
	 *
	 * @param stp
	 *            On entry this contains an initial estimate of a satifactory step
	 *            length. On exit <code>stp</code> contains the final estimate.
	 *
	 * @param ftol
	 ...
	 *
	 * @param maxfev
	 *            Termination occurs when the number of evaluations of the objective
	 *            function is at least <code>maxfev</code> by the end of an
	 *            iteration.
	 *
	 * @param info
	 ...
	 *            <li><code>info = -1</code> A return is made to compute the
	 ...
	 *            uncertainty is at most <code>xtol</code>.
	 *            <li><code>info = 3</code> Number of function evaluations has
	 *            reached <code>maxfev</code>.
	 ...
	 *            progress. There may not be a step which satisfies the sufficient
	 *            decrease and curvature conditions. Tolerances may be too small.
	 *            </ul>
	 *
	 * @param nfev
	 ...
	 * assumed that the derivative at <code>stx</code> is negative in the direction
	 * of the step. If <code>brackt[0]</code> is <code>true</code> when
	 ...
	 *
	 * @param stx
	 *            Step at the best step obtained so far. This variable is modified
	 *            by <code>mcstep</code>.
	 * @param fx
	 *            Function value at the best step obtained so far. This variable is
	 *            modified by <code>mcstep</code>.
	 * @param dx
	 *            Derivative at the best step obtained so far. The derivative must
	 *            be negative in the direction of the step, that is, <code>dx</code>
	 *            and <code>stp-stx</code> must have opposite signs. This variable
	 *            is modified by <code>mcstep</code>.
	 * 
	 * @param sty
	 *            Step at the other endpoint of the interval of uncertainty. This
	 ...
	 * 
	 * @param stp
	 *            Step at the current step. If <code>brackt</code> is set then on
	 *            input <code>stp</code> must be between <code>stx</code> and
	 ...
	 *            not been bracketed, then on input this variable must be set
	 *            <code>false</code>. If the minimizer has been bracketed, then on
	 *            output this variable is <code>true</code>.
	 * 
	 * @param stpmin
	 ...
	 *
	 * @author Jorge J. More, David J. Thuente: original Fortran version, as part of
	 *         Minpack project. Argonne Nat'l Laboratory, June 1983. Robert Dodier:
