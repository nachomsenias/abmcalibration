...
/**
 * This class applies gradient descent to a given solution until it
 ...
 * @author jjpalacios
 *
 * XXX IM It is possible that this class is not working properly. 
 *
 */
public class GradientDescent extends LocalSearch {
	...
	public void setup(final EvolutionState state, final Parameter base) {
     ...
     // Reads the type of neighbourhood to use
        ...
        if(neighborhood_str == null) {
        	state.output.fatal("The neighborhood for the gradient descent has "
        ...
        if(neighborhood_str.equalsIgnoreCase(P_NEIGH_TYPE1))
        	...
        	state.output.fatal("Incorrent value for the gradient descent "
	...
	public void apply(final EvolutionState state,
		...
		state.output.println("Entering Gradient Descent....", 0);
		double currentFitness = individual.fitness.fitness();
		...
		while(!stop && iterations < maxIterations
			...
			while(neighbor != null) {
				...
				nextNeighbor(state, individual);
			}
			if(improves) {
				moveToSolution(individual, bestNeighbor);
			}
			else {
				stop = true;
			}
		...
		state.output.println("Finishing Gradient Descent....", 0);
	...
	public void restartNeighborhood() {
		...
		for(int i=0; i < neighborhood.length; i++) {
			...
			int randInt = rng.nextInt(neighborhood.length);
	...
	protected Individual nextNeighborType1(final EvolutionState state,
		...
		if(individual instanceof IntegerVectorIndividual) {
			...
			if(neighborhood[neighborIndex] > 0) {	
				int index = neighborhood[neighborIndex]-1;
				if(neighbor.genome[index] < species.maxGene(index)-0.5) {
					neighbor.genome[index] += 1.0;
					neighbor.evaluated = false;
				}		
				// If there is no neighbor, move to the next one	
				else {
			...
			// Decrement a parameter
				...
				int index = -(neighborhood[neighborIndex]+1);
				if(neighbor.genome[index] > species.minGene(index)+0.5) {
					neighbor.genome[index] -= 1.0;
					neighbor.evaluated = false;
				}			
				// If there is no neighbor, move to the next one
